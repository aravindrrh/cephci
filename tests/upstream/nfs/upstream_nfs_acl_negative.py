"""
NFS v4 ACL negative tests against upstream nfs-ganesha (installer build).

Same scenarios as ``tests/nfs/acl/test_nfs_acl_negative.py`` using
``upstream_nfs_operations`` and the installer node.
...
def _report_results(results):
    ...
    if hard_failures:
        ...
        return 1
    if known_failures:
        log.info("Negative tests passed (failures match known issues only)")
        return 0
