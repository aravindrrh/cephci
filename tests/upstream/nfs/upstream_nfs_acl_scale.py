"""
NFS v4 ACL scale tests against upstream nfs-ganesha (installer build).

Same scenarios as ``tests/nfs/acl/test_nfs_acl_scale.py`` using
``upstream_nfs_operations``; remount uses the installer IP to match the
initial mount.
...
def run(ceph_cluster, **kw):
    ...
    # Remount must use the same address family as the initial mount (installer IP).
    nfs_mount_host = nfs_node.ip_address
...
def _report_results(results):
    ...
    if hard_failures:
        ...
        return 1
    if known_failures:
        log.info("Scale tests passed (failures match known issues only)")
        return 0
