"""
NFS v4 ACL functional tests against upstream nfs-ganesha (installer build).

Same scenarios as ``tests/nfs/acl/test_nfs_acl_functional.py``, but use
``upstream_nfs_operations`` (subvolumes + local ganesha.nfsd) and the
installer node instead of ceph orchestrator NFS daemons. Reboot persistence
is commented out for now (manual Ganesha is not restored after reboot; clients
would hang on stale mounts).
...
Runs all positive / feature-verification tests in a single pass:
  ...
  - Persistence (Rename, Hard link, Restart; reboot test commented out for manual upstream Ganesha)
"""

from time import sleep

# Reboot persistence block (commented below) would need:
# from ceph.waiter import WaitUntil
# from cli.utilities.utils import get_ip_from_node, reboot_node

from cli.exceptions import ConfigError
from tests.nfs.lib.nfs_acl import NfsAcl
from upstream_nfs_operations import cleanup_cluster, restart_upstream_ganesha, setup_nfs_cluster
...
# failures are logged as KNOWN and do not fail the overall test run.
...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        results.append(
            (
                "Restart Persistence",
                _run_test(_test_restart_upstream, acl, nfs_node),
            )
        )
        # Disabled for manual upstream Ganesha: reboot does not restore the daemon;
        # clients can hang on stale mounts. Re-enable when Ganesha is service-managed.
        # results.append(
        #     (
        #         "Reboot Persistence",
...
def _report_results(results):
    ...
    if hard_failures:
        ...
        return 1
    if known_failures:
        log.info("Functional tests passed (failures match known issues only)")
        return 0
...
def _test_restart_upstream(acl, installer_node):
    ...
    restart_upstream_ganesha(installer_node)
...
# ---------------------------------------------------------------------------
# Reboot persistence (disabled): manual upstream ganesha.nfsd is not restored
# after installer reboot; uncomment imports above and the results.append block
# in run() when the server runs Ganesha under a surviving supervisor.
# ---------------------------------------------------------------------------
