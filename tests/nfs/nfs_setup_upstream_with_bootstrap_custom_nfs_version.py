...
def run(ceph_cluster, **kw):
    ...
    custom_version = "refs/tags/5.7"  # refs/heads/next
    ...
    if checkout_patch:
        ...
        cmds = ['git clone --depth=1 https://github.com/nfs-ganesha/nfs-ganesha',
                f'cd $(basename "nfs-ganesha/nfs-ganesha"); git fetch origin {custom_version} && git checkout FETCH_HEAD; '
                ]
    cmds.append('cd $(basename "nfs-ganesha/nfs-ganesha"); git submodule update --recursive --init || git submodule sync ;')
    exec_cmds(installer, cmds)

    # Build nfs-ganesha
    cmds = ['cd $(basename "nfs-ganesha/nfs-ganesha"); [ -d build ] && rm -rf build ; mkdir build ; cd build ; '
